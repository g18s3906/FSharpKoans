...
module ``12: List operations are so easy, you could make them yourself!`` =
    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int = xs.Length
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
           let rec reverse xs list =
                match xs with
                |[]->list
                |a::xs->reverse xs (a::list)
           reverse xs []
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
           let rec add s =
             ...
             |a::rest -> a + 1:: add rest
             |[] -> []
           add xs// write a function which adds 1 to each element
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =            
           let rec f s =
             ...
             |a::rest -> a * 2:: f rest
             |[] -> []
           f xs         // write a function which doubles each element
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            List.fold(fun acc element -> acc + element) initialState xs    // write a function to do what's described above
